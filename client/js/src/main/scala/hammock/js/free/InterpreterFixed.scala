...
class InterpreterFixed[F[_]] extends InterpTrans[F] {
  ...
  private def doReq(req: HttpRequestF[HttpResponse], method: Method)(implicit S: Sync[F]): F[HttpResponse] = S.delay {
    ...
    val async = true // asynchronicity should be handled by the concurrency monad `F`, not the HTTP driver
...
object InterpreterFixed {
  def apply[F[_]]: InterpreterFixed[F] = new InterpreterFixed[F]
}
